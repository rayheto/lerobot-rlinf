...
**实验入口**：
- RLinf 驱动的 residual PPO 入口：`src/rl/rlinf_residual/train.py`。
  π₀.₅ 通过每 worker 一个 openpi server 保持冻结，只训练 residual MLP；
  已验证基线仍是 `src/rl/simple/`。
...
│       └── rlinf_residual/                      # RLinf residual PPO glue（实验）
│           ├── train.py                         # RLinf EmbodiedRunner 入口
│           ├── env.py                           # residual env：obs=(state, base), action=base+residual
│           ├── model.py                         # so101_residual_mlp 注册
...
└── third_party/
    ...
    └── RLinf/           (woshinideba1425 fork)                  — Phase 3 实验入口已接到 src/rl/rlinf_residual
```
...
| RLinf       | woshinideba1425 fork              | `src/rl/rlinf_residual` 中已有 experimental residual PPO 入口 |
...
| 3 | RLinf 驱动的 PPO/GRPO 后训练                                | experimental residual PPO glue 已接入 `src/rl/rlinf_residual` |
